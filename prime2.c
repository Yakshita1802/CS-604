...
_Bool is_prime(int x);  //without <stdboo.h>, boolean is defined by _Bool
...
int main() {
	...
	while (n!=-1) { //test
		...
		//int n;  -- this int n doen't required as int n is already defined in int (main)
...
_Bool is_prime(int n) {
	...
	for (int i = 2; i < n; i++) {
		if (n%i == 0) {
			return 0;      //without boolean library, false and true needed to be as 0 and 1 resp.
		}
	}
	return 1;
}
...
int get_valid_number(void) {
	...
	scanf("%d", &n);
	while (n <= 2 && n != -1) { //&& condition is used, as both <=2 and !=-1 condition need to be satisfied so while loop will return n, and then condition for prime can be check for n
		printf("Please enter an integer greater than 2 to test its primality, enter -1 to exit \n");
		scanf("%d", &n);
	}
	return n;
